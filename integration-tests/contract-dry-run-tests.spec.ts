...
CONFIGS().forEach(({ lib, setup, knownBaker, createAddress, protocol }) => {
  ...
  describe('Estimate scenario', () => {
    ...
    delphinet('Estimate transfer with allocated destination', async (done) => {
      const estimate = await LowAmountTez.dryRun.transfer({ to: await Tezos.signer.publicKeyHash(), amount: 1.9 });
      done();
    })

    carthagenet('Estimate transfer with allocated destination', async (done) => {
      const dryRun = await LowAmountTez.dryRun.transfer({ to: await Tezos.signer.publicKeyHash(), amount: 1.9 });
      done();
    })
    ...
    delphinet('Estimate simple origination', async (done) => {
      const estimate = await LowAmountTez.dryRun.originate({
        balance: "1",
        code: ligoSample,
        storage: 0,
      })
    ...
    delphinet('Estimate setDelegate', async (done) => {
      const estimate = await LowAmountTez.dryRun.setDelegate({
        delegate: knownBaker,
    ...
    delphinet('Estimate internal transfer to allocated implicit', async (done) => {
      ...
      const estimate = await LowAmountTez.dryRun.transfer(tx)
  ...
  describe('Estimate with very low balance', () => {
    ...
    delphinet('Estimate transfer to regular address', async (done) => {
      let estimate = await LowAmountTez.dryRun.transfer({ to: await Tezos.signer.publicKeyHash(), mutez: true, amount: amt - (1382 + DEFAULT_FEE.REVEAL) });
    ...
    it('Estimate transfer to regular address with a fixed fee', async (done) => {
      ...
      await expect(LowAmountTez.dryRun.transfer(params)).rejects.toEqual(
    ...
    it('Estimate transfer to regular address with unsufficient balance', async (done) => {
      await expect(
        LowAmountTez.dryRun.transfer({ to: await Tezos.signer.publicKeyHash(), mutez: true, amount: amt })
      ).rejects.toEqual(
        expect.objectContaining({
    ...
    it('Estimate transfer to regular address with unsufficient balance to pay storage for allocation', async (done) => {
      await expect(
        LowAmountTez.dryRun.transfer({ to: await (await createAddress()).signer.publicKeyHash(), mutez: true, amount: amt - (1382 + DEFAULT_FEE.REVEAL) })
      ).rejects.toEqual(
        expect.objectContaining({
    ...
    it('Estimate origination with unsufficient balance to pay storage', async (done) => {
      await expect(LowAmountTez.dryRun.originate({
        balance: "0",
        code: ligoSample,
        storage: 0,
      })).rejects.toEqual(
